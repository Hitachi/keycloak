...
NOTE: The OAuth Client ID Metadata Document feature is currently experimental. To enable it, start {project_name} with `--features=cimd`.

===== Setting up the ClientIdMetadataDocumentExecutor profile

The `ClientIdMetadataDocumentExecutor` is a client policy executor that processes an authorization request whose `client_id` parameter is a URL pointing to a Client ID Metadata Document.
...
===== Setting up the ClientIdUriSchemeCondition policy

The `ClientIdUriSchemeCondition` is a client policy condition that triggers when the `client_id` parameter in an authorization request is a URI matching a specified scheme (e.g., `https`). When this condition is met, the associated profiles (such as the `cimd-profile` configured above) are executed.
