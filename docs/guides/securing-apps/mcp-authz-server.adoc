...
* *Trusted domains*: A list of domain patterns (regex) that the executor accepts for the Client ID URL and Client Metadata URL properties. For example, use `(.*)\\.example\\.org` to accept any subdomain of `example.org`. If empty, all domains are denied.
...
https://code.visualstudio.com/[Microsoft Visual Studio Code] (VS Code) is an MCP client that supports OAuth Client ID Metadata Document. When VS Code connects to an MCP server that requires authorization, it sends an authorization request with a `client_id` parameter that is an `https` URL hosted on `vscode.dev` (e.g., `+https://vscode.dev/mcp-client+` or `+https://insiders.vscode.dev/mcp-client+`). {project_name} fetches the Client ID Metadata Document from this URL and uses the metadata to process the request.

To configure {project_name} for VS Code's MCP client, follow the steps below.

NOTE: VS Code is a public client that uses PKCE (Proof Key for Code Exchange) for OAuth. It does not use a client secret.
...
==== Setting up the client profile for VS Code
...
* *Trusted domains*: `(.*\.)?vscode\.dev` (this matches both `vscode.dev` and `insiders.vscode.dev`)
* *Restrict same domain*: `OFF` (VS Code desktop uses `+http://127.0.0.1+` as a redirect URI, which is not on the same domain as `vscode.dev`)
* *Only Allow Confidential Client*: `OFF` (VS Code is a public client)
+
. Click *Save*.

==== Setting up the client policy for VS Code
...
With this configuration, when VS Code sends an authorization request, {project_name} recognizes the `client_id` as a URL on `vscode.dev`, fetches the Client ID Metadata Document, and processes the request accordingly.
