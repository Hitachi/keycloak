...
public class IDJWTTokenExchangeProvider extends StandardTokenExchangeProvider {
    ...
    protected Response tokenExchange() {
        ...
        try {
            ...
            if (verifier.getHeader().getAlgorithm() == null) {
    ...
    protected Response buildTokenExchangeResponse(ClientSessionContext clientSessionCtx, String requestedTokenType, List<ClientModel> targetAudienceClients) {
        ...
        idjag.subject(getSubjectForTargetClient(targetAudienceClients.get(0), user));
        idjag.issuedNow();
        ...
        long expiration = Time.currentTimeSeconds() + lifespan;
        long sessionExpires = SessionExpirationUtils.calculateClientSessionMaxLifespanTimestamp(
            ...
            TimeUnit.SECONDS.toMillis(clientSession.getStarted()),
            TimeUnit.SECONDS.toMillis(userSession.getStarted()),
            realm,
            client);
        if (sessionExpires > 0) {
            expiration = Math.min(expiration, TimeUnit.MILLISECONDS.toSeconds(sessionExpires));
        }
        idjag.exp(expiration);
        ...
        res.setExpiresIn(idjag.getExp() - Time.currentTime());
    ...
    private String getSubjectForTargetClient(ClientModel targetClient, UserModel user) {
        ProtocolMapperModel mapperModel = Stream.concat(
                Stream.of(targetClient),
                targetClient.getClientScopes(true).values().stream()
            )
            .flatMap(mapperContainer -> mapperContainer.getProtocolMappersStream())
            .filter(mapper -> OIDCLoginProtocol.LOGIN_PROTOCOL.equals(mapper.getProtocol()))
            .filter(mapper -> mapper.getProtocolMapper().endsWith(AbstractPairwiseSubMapper.PROVIDER_ID_SUFFIX))
            .findFirst()
            .orElse(null);
        ...
        if (!(mapper instanceof AbstractPairwiseSubMapper)) {
            throw new IllegalStateException(
                "Configured mapper is not a pairwise subject mapper: " + mapperModel.getProtocolMapper());
        }

        String configuredSectorUri = PairwiseSubMapperHelper.getSectorIdentifierUri(mapperModel);
        String sectorIdentifier = configuredSectorUri == null || configuredSectorUri.isBlank()
            ? PairwiseSubMapperUtils.resolveValidSectorIdentifier(targetClient.getRootUrl(), targetClient.getRedirectUris())
            : PairwiseSubMapperUtils.resolveValidSectorIdentifier(configuredSectorUri);

        if (sectorIdentifier == null || sectorIdentifier.isBlank()) {
            throw new IllegalStateException(
                "Could not resolve sector identifier for target client: " + targetClient.getClientId());
        }

        return ((AbstractPairwiseSubMapper) mapper).generateSub(mapperModel, sectorIdentifier, user.getId());
    }
